...
export async function POST(
  ...
  if (!profileA || !profileB) {
    return NextResponse.json({ error: 'Profiles not complete' }, { status: 422 })
  }

  const suggestion = await aiService.getConversationSuggestion({
    matchId,
    profileA: profileToPersonality(profileA),
    profileB: profileToPersonality(profileB),
    compatibilityData: {
    ...
    conversationHistory: match.messages.map((m) => ({
      id: m.id,
      senderId: m.senderId,
      content: m.content,
